...
9. 教师上传前必须确认：拥有处理授权、尽量遮盖姓名、学校和联系方式；图片会发送给配置的第三方 AI。已上传作文在本机长期保留，教师可在历史记录中手动永久删除，但这不代表第三方服务中的数据已同步删除。
10. `npm run retention -- inspect` 仅检查超过 24 小时仍未上传图片的空草稿，以及此前删除失败、等待重试的记录。确认结果后可运行 `npm run retention -- run` 完成空草稿清理和手动删除重试；不得把这个命令当作已上传作文的定期到期清理。
11. 每周检查 `.data/app.db`、`.data/reviews/` 和磁盘剩余容量，为存储容量设置告警。根据学校的数据管理要求使用加密、受控且经过恢复演练的备份；备份也必须纳入手动永久删除后的数据处置流程。
12. 更新版本前先停止服务，等待当前 Worker 完成，再拉取代码、构建并重新启动。管理员可停用账号、重置密码和撤销会话。
